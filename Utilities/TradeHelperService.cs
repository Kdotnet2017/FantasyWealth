...
namespace FantasyWealth.Utilities
{
    ...
    public class TradeHelperService : Controller
    {
        ...
        public async Task<string> apiWealthFind(int symbolId)
        {
            ...
            _logger.LogWarning("Json Result : " + wealth.Count().ToString());
            if (wealth.Count() > 0)
            {
        ...
        public async Task<Wealth> isWealthOwner(int symbolId, string userId)
        {
            ...
            .Where(w => w.UserId == userId && w.SymbolId == symbolId)
            .FirstOrDefaultAsync();
            _logger.LogWarning(symbolId.ToString() + " user : " + userId + " ///// " + wealth);
            if (wealth == null)
            {
                return null;
            }
            ...
            if (user.Id == transaction.UserId)
            {
                ...
                switch (switchType)
                {
                    case 0:  //Transfer
                        ...
                        {
                            _logger.LogWarning("You cannot do this transfer.");
                            // Send message
                            // Do nothing
